...
def fixed_horizon_validation(
    ...
    for horizon in horizons:
        ...
        snapshots = record.get("strategy_signals", [])
        validations[str(horizon)] = {
            ...
            "beat_spy": bool(alpha > 0),
            "strategies": {
                snapshot["strategy"]: {
                    "signal": snapshot.get("signal"),
                    "stock_return_pct": round(float(stock_return) * 100, 2),
                    "spy_return_pct": round(float(spy_return) * 100, 2),
                    "alpha_vs_spy_pct": round(float(alpha) * 100, 2),
                    "beat_spy": bool(alpha > 0),
                }
                for snapshot in snapshots
                if snapshot.get("signal") == "BUY"
            },
        }
    return validations
...
def strategy_scorecard(records: list[dict], horizon: int = 20) -> pd.DataFrame:
    ...
    for name in names:
        ...
        stock_returns = [sample["stock_return_pct"] for sample in samples]
        spy_returns = [sample["spy_return_pct"] for sample in samples]
        rows.append({
            "策略": name,
            "已驗證訊號": len(samples),
            "勝過 SPY": sum(value > 0 for value in alpha_spy),
            "勝過 SPY 比率 %": 100 * sum(value > 0 for value in alpha_spy) / len(samples),
            "平均股票報酬 %": sum(stock_returns) / len(stock_returns),
            "平均 SPY 報酬 %": sum(spy_returns) / len(spy_returns),
