    ...
    for i in range(len(data)):
        ...
        if i > 0:
            ...
            if prev["signal"] == "sell" and shares > 0:
                cash, px = _sell(shares, float(row["open"]), cfg)
                trades.append({"signal_date": prev["date"], "execution_date": current_date,
                               "date": current_date, "action": "sell", "execution_price": px,
                               "shares": shares, "reason": "strategy_signal"})
                shares, entry_price, highest_known, entry_signal_date = 0.0, None, None, None
            elif prev["signal"] == "buy" and cash > 0:
                shares, px = _buy(cash, float(row["open"]), cfg)
                cash, entry_price, highest_known, entry_signal_date = 0.0, px, px, prev["date"]
                trades.append({"signal_date": prev["date"], "execution_date": current_date,
                               "date": current_date, "action": "buy", "execution_price": px,
                               "shares": shares, "reason": "strategy_signal"})
        ...
        if shares > 0 and entry_price is not None:
            if cfg.exit_mode == "take_profit":
                ...
                if float(row["high"]) >= target:
                    cash, px = _sell(shares, target, cfg)
                    trades.append({"signal_date": entry_signal_date, "execution_date": current_date,
                                   "date": current_date, "action": "sell", "execution_price": px,
                                   "shares": shares, "reason": f"take_profit_{cfg.take_profit_pct:g}pct"})
                    shares, entry_price, highest_known, entry_signal_date = 0.0, None, None, None
            elif cfg.exit_mode == "trailing":
                # Conservative daily-OHLC rule: today's new high cannot raise today's stop.
                stop = highest_known * (1 - cfg.trailing_pct / 100)
                if float(row["low"]) <= stop:
                    cash, px = _sell(shares, stop, cfg)
                    trades.append({"signal_date": entry_signal_date, "execution_date": current_date,
                                   "date": current_date, "action": "sell", "execution_price": px,
                                   "shares": shares, "reason": f"trailing_{cfg.trailing_pct:g}pct"})
                    shares, entry_price, highest_known, entry_signal_date = 0.0, None, None, None
                else:
                    highest_known = max(highest_known, float(row["high"]))

        position_value = shares * float(row["close"])
        equity_rows.append({"date": current_date, "close": float(row["close"]), "cash": cash,
                            "shares": shares, "position_value": position_value,
                            "portfolio_value": cash + position_value, "position": int(shares > 0)})

    equity_curve = pd.DataFrame(equity_rows)
    final_value = float(equity_curve["portfolio_value"].iloc[-1])
    return {"initial_capital": cfg.initial_capital, "final_value": round(final_value, 2),
            "total_return_pct": round((final_value / cfg.initial_capital - 1) * 100, 2),
            "number_of_trades": len(trades), "transaction_cost_pct": cfg.transaction_cost_pct,
            "slippage_pct": cfg.slippage_pct, "trades": trades,
            "still_holding_shares": shares > 0, "equity_curve": equity_curve,
            "exit_mode": cfg.exit_mode}

def calculate_buy_and_hold(df, initial_capital=10000, transaction_cost_pct=0.001, slippage_pct=0.0005):
    if df.empty:
        raise ValueError("Buy & Hold data is empty.")
    first_open, final_close = float(df["open"].iloc[0]), float(df["close"].iloc[-1])
    ...
    return {"initial_capital": initial_capital, "final_value": round(final_value, 2),
            "total_return_pct": round((final_value / initial_capital - 1) * 100, 2),
            "start_date": df["date"].iloc[0], "end_date": df["date"].iloc[-1],
            "start_price": first_open, "end_price": final_close,
            "transaction_cost_pct": transaction_cost_pct, "slippage_pct": slippage_pct}
