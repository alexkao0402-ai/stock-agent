...
class BacktestEngineTests(unittest.TestCase):
    def test_signal_executes_next_day_open(self):
        ...
        result = run_backtest(df, BacktestConfig(initial_capital=1000, transaction_cost_pct=0, slippage_pct=0))
    ...
    def test_future_open_does_not_change_prior_day_equity(self):
        ...
        self.assertEqual(r1["equity_curve"].loc[1, "portfolio_value"], r2["equity_curve"].loc[1, "portfolio_value"])
    ...
    def test_trailing_stop_does_not_use_same_day_new_high(self):
        ...
        result = run_backtest(df, BacktestConfig(initial_capital=1000, transaction_cost_pct=0, slippage_pct=0, exit_mode="trailing", trailing_pct=20))
    ...
    def test_take_profit_has_equity_curve_and_execution_date(self):
        ...
        result = run_backtest(df, BacktestConfig(initial_capital=1000, transaction_cost_pct=0, slippage_pct=0, exit_mode="take_profit", take_profit_pct=25))
        sells = [t for t in result["trades"] if t["action"] == "sell"]
    ...
    def test_buy_and_hold_applies_cost_and_slippage(self):
        ...
        frictionless = calculate_buy_and_hold(df, initial_capital=1000, transaction_cost_pct=0, slippage_pct=0)
        with_costs = calculate_buy_and_hold(df, initial_capital=1000, transaction_cost_pct=0.001, slippage_pct=0.0005)
